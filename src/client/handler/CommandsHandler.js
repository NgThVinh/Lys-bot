...
class CommandsHandler {
  ...
  load = () => {
    for (const directory of readdirSync('./src/commands/')) {
      for (const file of readdirSync('./src/commands/' + directory).filter(f =>
        f.endsWith('.js')
      )) {
        try {
          /**
          ...
          error(
            'Unable to load a command from the path: ' + 'src/commands/' + directory + '/' + file
          );
        }
      }
    }
  ...
  registerApplicationCommands = async (development, restOptions = null) => {
    const rest = new REST(restOptions ? restOptions : { version: '10' }).setToken(
      this.client.token
    );
